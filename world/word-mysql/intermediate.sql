...
-- en los países que sí lo hablan de la región 'Caribbean'?
...
SELECT 
	...
	(SELECT COUNT(*)
		FROM country c, countrylanguage l
		WHERE c.code = l.countrycode
		AND l.language = 'English') as count;
...
-- Query #5 step by step
			
SELECT continent, COUNT(name) AS subtotal
	FROM country
	GROUP BY continent;
...
SELECT continent,
	SUM(CASE WHEN lifeexpectancy IS NULL THEN 1 ELSE 0 END) nula,
	SUM(CASE WHEN lifeexpectancy<50 THEN 1 ELSE 0 END) baja,
	SUM(CASE WHEN (lifeexpectancy>=50 AND lifeexpectancy<65) 
		THEN 1 ELSE 0 END) media,
	SUM(CASE WHEN lifeexpectancy>=65 THEN 1 ELSE 0 END) alta,
	COUNT(name) AS subtotal
	FROM country
	GROUP BY continent;
